...
### 前提: Garmin Connect 側での入力が要る

2026-08-07 のボルダリング（手動追加）を確認したところ `directWorkoutRpe` は `null` だった。
**アクティビティを編集して「主観的運動強度」を入れないと値は入らない。**

入れ忘れた日は RPE が `null` になる。負荷判定はその日を「不明」として扱い、
推測で埋めない。
