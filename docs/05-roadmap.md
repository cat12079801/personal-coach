...
先に **0008 の適用**がある。以降は優先度順ではなく、どれも任意。

- **マイグレーション `0008_drop_skating_logs_and_unlinked.sql` の適用** — 未適用。
  流すまで `/logs` の「最近の記録」は動くが、使っていない `skating_logs` と
  `unlinked_activities` が DB に残る
