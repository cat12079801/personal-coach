...
### CI では流さない（2026-08-13 決定）

**マイグレーションは手で流す。** GitHub Actions から自動適用しない。

相乗り先（count-upper）のマイグレーション履歴がこのリポジトリに無いため、
`supabase db push` は共有の履歴テーブルを見て予期しない差分を出しうる。
public リポジトリから本番 DB へ DDL を流す経路自体を持たない方が事故が少ない。

自前の適用台帳（`coach.schema_migrations`）を持って `psql` で流す案も検討したが、
**DDL を自動で流さないという判断を優先して採らない**。
